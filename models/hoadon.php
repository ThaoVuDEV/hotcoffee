...
function insert_hoadon($id_tk, $tong, $pttt, $username, $email, $sdt, $address)
{
    ...
    $sql = "INSERT INTO `hoa_don` (`id_tk`, `tong_tien`, `phuong_thuc_tt`, `user`, `email`, `sdt`, `dia_chi`, `ngay_dat`, `trang_thai`) 
            VALUES (:id_tk, :tong_tien, :pttt, :username, :email, :sdt, :address, CURDATE(), :trang_thai)";

    $stmt = $conn->prepare($sql);

    $tong = preg_replace('/[^0-9]/', '', $tong);
    $tong = (int) $tong;

    $stmt->bindParam(':id_tk', $id_tk);
    $stmt->bindParam(':tong_tien', $tong);
